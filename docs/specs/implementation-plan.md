...
- `backend/app/services/text_processing.py`:
  ...
  - Основной подход: semantic-shift сегментация (эмбеддинги соседних сегментов + адаптивный порог смены темы)
  - Возвращает: `[{timecode_start, timecode_end, text}, ...]`

**Критерии готовности:**
- Текст разбивается на логические тематические блоки (смена смысла/темы)
- Длина блока контролируется как мягкое ограничение (ориентир ~100-300 слов, но без жесткого принуждения)
